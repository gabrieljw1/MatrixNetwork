...
public class ArrayUtil {
	...
	/**
	 * Splits off columns of an array as opposed to clipArray which splits of rows
	 * 
	 * @param array
	 * @param startingIndex
	 * @param endingIndex
	 * @return
	 */
	public static double[][] splitArray(double[][] array, int startingIndex, int endingIndex) {
		double[][] returnArray = new double[array.length][endingIndex - startingIndex];
		
		for (int i = 0; i < array.length; i++) {
			for (int j = startingIndex; j < returnArray[i].length + startingIndex; j++) {
				returnArray[i][j - startingIndex] = array[i][j];
			}
		}
	...
	public static double[] transposeColumnArray(double[][] array) {
		if (array.length == 0 || array[0].length > 1) { return null; }
	...
	public static double[] generateRandomArray(int length,  int maxInt) {
		Random random = new Random();
		
		double[] returnArray = new double[length];
		
		for (int i = 0; i < returnArray.length; i++) {
			returnArray[i] = random.nextInt(maxInt);
		}
		
		return returnArray;
	}
	
	public static double[][] generateRandomArray(int length, int width, int maxInt) {
		double[][] returnArray = new double[length][];
		
		for (int i = 0; i < returnArray.length; i++) {
			returnArray[i] = generateRandomArray(width, maxInt);
		}
		
		return returnArray;
	}
	
	public static double[] absArray(double[] array) {
		double[] returnArray = new double[array.length];
